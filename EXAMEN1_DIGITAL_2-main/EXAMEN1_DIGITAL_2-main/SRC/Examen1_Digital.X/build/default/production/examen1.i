...
INISYS:
    ...
    BSF STATUS, 5 ; Banco1
    ;Modificar TRIS
    BSF TRISD,0 ;PORTD,0 ;sensor de nivel=PORTD0->ENTRADA SENSOR DE NIVEL
    BSF TRISD,1 ;PORTD,1 ;sensor de temperatura=PORTD1->ENTRADA SENSOR DE TEMPERATURA
    BSF TRISD,2 ;PORTD,2 ;oxi1 es el bit mas significativo de mi sensor de oxigeno=PORTD2->ENTRADA BIT MAS SIGNIFICATIVO SENSOR DE OXIGENO
    BSF TRISD,3 ;PORTD,3 ;oxi2 es el bit menos significativo de mi sensor de oxigeno=PORTD3->ENTRADA BIT MENOS SIGNIFICATIVO SENSOR DE OXIGENO
    BCF TRISC,0 ;PORTC,0 ;bomba = led amarillo=PORTC0->SALIDA BOMBA DE AGUA
    BCF TRISC,1 ;PORTC,1 ;resistencia electrica de calefaccion = led rojo=PORTC1->SALIDA RESISTENCIA DE CALEFACCION
    BCF TRISC,2 ;PORTC,2 ;PORTC,0 ;bomba = led amarillo de aire = led verde=PORTC2->SALIDA BOMBA DE AIRE
    ; Regresar a banco 0
...
MAIN:
    ...
    PUNTO1:
    BTFSC PORTD,0 ;sensor de nivel
    GOTO OFFBOMBA ;SI D0=1
    GOTO ONBOMBA ;SI D0=0
    ONBOMBA:
    BSF PORTC,0 ;bomba = led amarillo ;ENCIENDE LA BOMBA DE AGUA (LED AMARILLO)
    GOTO PUNTO2
    OFFBOMBA:
    BCF PORTC,0 ;bomba = led amarillo ;APAGA LA BOMBA DE AGUA (LED AMARILLO)
    GOTO PUNTO2


    PUNTO2:
    BTFSC PORTD,1 ;sensor de temperatura
    GOTO OFFRESIST ;SI D1=1
    GOTO ONRESIST ;SI D1=0
    ONRESIST:
    BSF PORTC,1 ;resistencia electrica de calefaccion = led rojo ;ENCIENDE LA RESISTENCIA (LED ROJO)
    GOTO PUNTO3
    OFFRESIST:
    BCF PORTC,1 ;resistencia electrica de calefaccion = led rojo ;APAGA LA RESISTENCIA (LED ROJO)
    GOTO PUNTO3

    PUNTO3:
    BTFSC PORTD,2 ;oxi1 es el bit mas significativo de mi sensor de oxigeno ;SOLO PREGUNTO POR EL BIT MAS SIGNIFICATIVO PORQUE SI ESTA EN CERO QUIERE DECIR QUE LE FALTA OXIGENO
    GOTO OFFAIRE ;SI D2=1
    GOTO ONAIRE ;SI D2=0
    ONAIRE:
    BSF PORTC,2 ;PORTC,0 ;bomba = led amarillo de aire = led verde ;ENCIENDE LA BOMBA DE AIRE (LED VERDE)
    GOTO MAIN
    OFFAIRE:
    BCF PORTC,2 ;PORTC,0 ;bomba = led amarillo de aire = led verde ;APAGA LA BOMBA DE AIRE (LED VERDE)
    GOTO MAIN
